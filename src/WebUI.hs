           ...
           defaultETagContext True >>= \ctx -> Web.scotty pnum $ do
                  ...
                  Web.get "/assess" $ do
                          ...
                          let as  = blankAssessment v student' teacher'
                              ...
                              js  = Text.intercalate " " [ "var $ = document.querySelectorAll.bind(document);"
                                                         ...
                                                         ,     "as[i].className = 'tab';"
                                                         ,   "}"
                                                         ,   "$('#' + id)[0].className += ' selected';"
                                                         ,   "numberRows();"
                                                         , "}"
                                                         , "function copyScore(id,val) {"
                                                         ,   "var exs = $('[type=radio][name=\"' + id + '\"');"
                                                         ,   "for (i = 0; i < exs.length; i++) {"
                                                         ,     "exs[i].value = val;"
                                                         ,   "}"
                                                         , "}"
                                                         , "function copyAdapt(self,id) {"
                                                         ,   "var exs = $('[type=checkbox][name=\"' + id + '\"');"
                                                         ,   "for (i = 0; i < exs.length; i++) {"
                                                         ,     "exs[i].checked = self.checked;"
                                                         ,   "}"
                                                         , "}"
                                                         , "function numberRows() {"
                                                         ,   "var trs = $('tr:not(#heading)');"
                                                         ,   "for (i = 0, c = 1; i < trs.length; i++) {"
                                                    ...
                                                    ul_ $ do li_ $ do a_ [href_ "http://www.ubuntu.com/", class_ "inline"] "Ubuntu"
                                                             ...
                                                             li_ $ do "A complete list of our Haskell library dependencies is available in the table below."
                                                                      table_ $ do tr_ $ do td_ "Name"; td_ "Description"; td_ "License"
                                                                                  ...
                                                                                  tr_ $ do td_ $ a_ [href_ "https://hackage.haskell.org/package/wai-middleware-static", class_ "inline"] "Wai-Middleware-Static"
                                                                                           td_ "For a middleware to host our static assets with Scotty"
                                                                                           td_ $ a_ [href_ "https://hackage.haskell.org/package/wai-middleware-static-0.8.0/src/LICENSE", class_ "inline"] "BSD3"
                                                                                  tr_ $ do td_ $ a_ [href_ "https://hackage.haskell.org/package/wai-middleware-etag", class_ "inline"] "Wai-Middleware-ETag"
                                                                                           td_ "For a middleware to automatically handle asset caching with Scotty"
                                                                                           td_ $ a_ [href_ "https://hackage.haskell.org/package/wai-middleware-etag-0.1.1.1/src/LICENSE", class_ "inline"] "BSD3"
